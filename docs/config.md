...
`quick.config.js`中包含了`quick-cli`的打包配置和文档站点配置，请创建此文件并置于项目根目录下。下面是一份基本配置的示例：
...
module.exports = {
  ...
  site: {
    ...
    title: 'Demo UI',
    // 图标
    logo: 'https://img01.yzcdn.cn/quick/logo.png',
    // 描述
    description: '示例组件库',
    // 左侧导航
    nav: [
...
module.exports = {
  site: {
    versions: [
      {
        label: 'v1',
        link: 'https://youzan.github.io/quick/v1/',
      },
    ],
  },
};
...
推荐使用`quick-cli`内置的 preset，配置如下：

```js
module.exports = {
  presets: ['@quick/cli/preset'],
};
```

`@quick/cli/preset`中默认包含了以下插件：
